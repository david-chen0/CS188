    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            for action in self.mdp.getPossibleActions(state):
                for successor, __ in self.mdp.getTransitionStatesAndProbs(state, action):
        ...
        for state in self.mdp.getStates():
            if self.mdp.isTerminal(state):
                continue
            action = self.computeActionFromValues(state)
            diff = abs(self.values[state] - self.computeQValueFromValues(state, action))
        ...
        for __ in range(self.iterations):
            ...
            for predecessor in predecessors[state]:
                ...
                diff = abs(self.values[predecessor] - self.computeQValueFromValues(predecessor, action))
                if diff > self.theta:
